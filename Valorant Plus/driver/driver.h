...
#define readioctlnewscarlet CTL_CODE(FILE_DEVICE_UNKNOWN, 0xE4308B, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
...
FARPROC pDeviceIoControl = GetProcAddress(hKernel32, "DeviceIoControl");
...
DeviceIoControl_t myDeviceIoControl = reinterpret_cast<DeviceIoControl_t>(pDeviceIoControl);

void custom_device_control(HANDLE hDevice,
	DWORD        dwIoControlCode,
	LPVOID       lpInBuffer,
	DWORD        nInBufferSize,
	LPVOID       lpOutBuffer,
...
lexemvedriver_handle = CreateFileW(_(L"\\\\.\\\VGK_G091H"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
	...
	void lexemvewrite_physical(PVOID address, PVOID buffer, DWORD size) {
		...
		custom_device_control(lexemvedriver_handle, readioctlnewscarlet, &arguments, sizeof(arguments), nullptr, NULL, NULL, NULL); // read-write
	}

	void lexemveread_physicaqrwetwel(PVOID address, PVOID buffer, DWORD size) {
...
T lxmvwrte(uint64_t address, T buffer) {
	...
	mem::lexemvewrite_physical((PVOID)address, &buffer, sizeof(T));
	return buffer;
}
...
T lexemveread2(uint64_t address) {
	T buffer{ };
	mem::lexemveread_physicaqrwetwel((PVOID)address, &buffer, sizeof(T));
